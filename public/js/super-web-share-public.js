...
function hasPermission() {
  if( ( typeof navigator.share==='undefined' || !navigator.share ) && window.superWebShareFallback.superwebshare_fallback_enable == 'disable' ){
	  console.log(window.superWebShareFallback.superwebshare_fallback_enable);
...
async function SuperWebSharefn(Title,URL,Description){
  if(typeof navigator.share==='undefined' || !navigator.share){
    modal();
  } else if(window.location.protocol!='https:' && 0 ){
...
document.addEventListener('click', function(SuperWebShare) {
  ...
  else if( target.classList.contains( 'modal-bg' ) ){
    modal('hide');
  }
});
